...
Expo Router supports three output targets for web apps. In SDK 58 and later, projects that depend on `expo-router` default to `server` when `web.output` is omitted. Projects without Expo Router default to `single`. Explicit output settings take precedence.

| Output   | Expo Router | API Routes         | Description                                                                                                                                               |
| -------- | ----------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `single` | <YesIcon /> | <NoIcon />         | Outputs a Single Page Application (SPA) with a single **index.html** in the output directory and has no statically indexable HTML.                        |
| `server` | <YesIcon /> | Enabled by default | Creates **client** and **server** directories with client assets, API routes, and a server that renders HTML for each request. Requires a server runtime. |
| `static` | <YesIcon /> | Opt-in             | Outputs separate HTML files for every route in the **app** directory.                                                                                     |

In SDK 58 and later, set `apiRoutes: true` in the `expo-router` config plugin to enable API routes with `static` output. API routes are enabled by default with `server` output. Set `apiRoutes: false` to disable them in either mode. See [API routes](/router/web/api-routes/#create-an-api-route) for configuration and deployment requirements.
...
The static hosting configurations below require `web.output` to be set to `static` or `single`. For server output, use [EAS Hosting](/eas/hosting) or follow the [server deployment guide](/router/web/server-rendering/#deployment).
...
2. When prompted **Configure as a single-page app (rewrite all urls to /index.html)**, only select **Yes** if you used `web.output: "single"`. Otherwise, select **No**.
