import type { TypeNode } from '../../scanner/types';
...
describe(renderType, () => {
  ...
  it('renders optionals as a null union', () => {
    ...
    expect(renderType({ kind: 'optional', wrapped: Str }, ctx)).toBe('string | null');
  });

  it('renders arrays and parenthesizes union elements', () => {
    const { ctx } = makeContext();
    expect(renderType({ kind: 'array', element: Int }, ctx)).toBe('number[]');
    expect(renderType({ kind: 'array', element: { kind: 'optional', wrapped: Str } }, ctx)).toBe(
      '(string | null)[]'
    );
  });
  ...
  it('renders dictionaries as Record', () => {
    const { ctx } = makeContext();
    expect(renderType({ kind: 'dictionary', key: Str, value: Int }, ctx)).toBe(
  ...
  it('renders promises', () => {
    const { ctx } = makeContext();
    expect(renderType({ kind: 'promise', value: Int }, ctx)).toBe('Promise<number>');
  });
  ...
  it('renders closures with positional argument names', () => {
    ...
    expect(
      renderType(
        { kind: 'function', parameters: [Int, Str], returns: Bool, async: false, throws: false },
        ctx
      )
    ).toBe('(arg0: number, arg1: string) => boolean');
    expect(
      renderType({ kind: 'function', parameters: [Double], async: false, throws: false }, ctx)
  ...
  it('wraps an async closure result in a promise', () => {
    const { ctx } = makeContext();
    expect(
      renderType({ kind: 'function', parameters: [], returns: Int, async: true, throws: true }, ctx)
    ).toBe('() => Promise<number>');
    expect(renderType({ kind: 'function', parameters: [], async: true, throws: false }, ctx)).toBe(
      '() => Promise<void>'
    );
  });
  ...
  it('resolves refs to scanned types under their JS name', () => {
    ...
    expect(renderType({ kind: 'ref', name: 'VideoPlayer' }, ctx)).toBe('Player');
    expect(warnings).toEqual([]);
  });

  it('renders an unresolved ref as unknown and warns', () => {
    const { ctx, warnings } = makeContext();
    expect(renderType({ kind: 'ref', name: 'PlaybackStatus' }, ctx)).toBe('unknown');
